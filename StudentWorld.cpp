...
int StudentWorld::virusCanFindFood(Actor *actor, double x, double y){
    double cloest=-1;
    ...
    if((actor->getY()-newY)<0){
        ...
        directionAngle=270-atan((actor->getX()-newY)/(actor->getY()-newY))*180/PI;
    }
    else{
...
void StudentWorld::createItems(char ch, double x,double y,Direction dir){
    switch (ch) {
        case 'F':
...
void StudentWorld::createVirus(char ch, double x, double y){
    switch(ch){
        case 'R':
